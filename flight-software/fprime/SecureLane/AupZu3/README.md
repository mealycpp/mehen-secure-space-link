...
## Building and Running the AupZu3 Application

In order to build the AupZu3 application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the AupZu3 application's code.
```
fprime-util build
```
...
To run the ground system without starting the AupZu3 app:
```
...
```
cd AupZu3/build-artifacts/<platform>/bin/
./AupZu3 -a 127.0.0.1 -p 50000
```
